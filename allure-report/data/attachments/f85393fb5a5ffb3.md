...
- Location: tests\bug5-sorting.spec.js:15:5
...
```ts
  ...
  5  | 
  6  | /**
  7  |  * ТЕСТ #5: Поиск бага с сортировкой
  8  |  * 
  9  |  * БАГ: Сортировка "Name (Z to A)" работает некорректно для problem_user
  10 |  * 
  11 |  * Ожидаемое поведение: Товары сортируются в обратном алфавитном порядке
  12 |  * Фактическое поведение: Порядок товаров не меняется или меняется неправильно
  13 |  */
  14 | test.describe('Bug #5: Sorting malfunction', () => {
  15 |     test('Problem user should see correctly sorted items (Z to A)', async ({ page }) => {
  16 |         allure.feature('Sorting');
  17 |         allure.story('Product sorting');
  18 |         allure.severity('normal');
  19 |         allure.tag('bug','sorting','ui');
  20 |         allure.description(`
  21 |             Given я на странице каталога как problem_user
  22 |             When выбираю сортировку "Name (Z to A)"
  23 |             Then товары должны отобразиться в обратном алфавитном порядке
  24 |             BUT порядок товаров остаётся как при сортировке A to Z
  25 |         `);
  26 | 
  27 |         const loginPage = new LoginPage(page);
  28 |         const inventoryPage = new InventoryPage(page);
  29 | 
  30 |         await loginPage.goto();
  31 |         await loginPage.login('problem_user', 'secret_sauce');
  32 |         await inventoryPage.waitForInventoryLoaded();
  33 | 
  34 | 
  35 |         // Сортируем от а до я
  36 | 
  37 |         await inventoryPage.sortBy('za');
  38 | 
  39 |         // Получаем список названий
  40 | 
  41 |         const itemNames = await inventoryPage.getItemNames();
  42 | 
  43 |         // Создаем ожидаемый отсортированный список
  44 | 
  45 |         const expectedSorted = [...itemNames].sort().reverse();
  46 | 
  47 |         // Прикрепляем оба списка к отчету
  48 | 
  49 |         await test.info().attach('actual-order', {
  50 |             body: `Фактический порядок ${itemNames}.join(', ')}`,
  51 |             contentType: 'text/plain'
  52 |         });
  53 | 
  54 |         await test.info().attach('expected-order', {
  55 |             body: `Ожидаемый порядок ${expectedSorted}.join(', ')}`,
  56 |             contentType: 'text/plain'
  57 |         });
  58 | 
  59 |         // Ожидаем, что порядок соответствует обратному алфавитному
  60 |         // ТЕСТ УПАДЁТ для problem_user
> 61 |         expect(itemNames).toEqual(expectedSorted);
     |         ^ ReferenceError: expect is not defined
  62 | 
  63 |         const screenshot = await page.screenshot();
  64 |         await test.info().attach('sorting-bug', {
  65 |             body: screenshot,
  66 |             contentType: 'image/png',
  67 |             description:'Сортировка Z→A не изменила порядок товаров'
  68 |         });
  69 | 
  70 |         console.log('БАГ НАЙДЕН: Сортировка работает некорректно для problem_user');
  71 |     });
  72 | });
```